...
public final class GCMRegistrar
{
	...
	public static void checkManifest(Context context)
	{
		...
		if (LogHelper.DEBUG_MODE)
		{
			LogHelper.v(TAG, "number of receivers for " + packageName + ": " + receivers.length);
		}
	...
	private static void checkReceiver(Context context, Set<String> allowedReceivers, String action)
	{
		...
		if (LogHelper.DEBUG_MODE)
		{
			LogHelper.v(TAG, "Found " + receivers.size() + " receivers for action " + action);
		}
	...
	static void internalRegister(Context context, String... senderIds)
	{
		...
		LogHelper.v(TAG, "Registering app " + context.getPackageName() + " of senders " + flatSenderIds);
	...
	public static synchronized void onDestroy(Context context)
	{
		...
		{
			LogHelper.v(TAG, "Unregistering receiver");
	...
	{
		LogHelper.v(TAG, "Unregistering app " + context.getPackageName());
	...
	static synchronized void setRetryBroadcastReceiver(Context context)
	{
		if (sRetryReceiver == null)
		{
			...
			{
				// should never happen
				LogHelper.d(TAG, "internal error: retry receiver class not set yet");
			...
			else
			{
				...
				{
					LogHelper.e(TAG, "Could not create instance of " + sRetryReceiverClassName + ". Using " + GCMBroadcastReceiver.class.getName()
							+ " directly.", e);
					sRetryReceiver = new GCMBroadcastReceiver();
				}
			}
			...
			LogHelper.v(TAG, "Registering receiver");
	...
	{
		LogHelper.v(TAG, "Setting the name of retry receiver class to " + className);
	...
	public static String getRegistrationId(Context context)
	{
		...
		{
			LogHelper.v(TAG, "App version changed from " + oldVersion + " to " + newVersion + "; resetting registration id");
	...
	static String setRegistrationId(Context context, String regId)
	{
		...
		LogHelper.v(TAG, "Saving regId on app version " + appVersion);
	...
	public static void setRegisteredOnServer(Context context, boolean flag)
	{
		...
		LogHelper.v(TAG, "Setting registeredOnServer status as " + flag + " until " + new Timestamp(expirationTime));
	...
	public static boolean isRegisteredOnServer(Context context)
	{
		...
		LogHelper.v(TAG, "Is registered on server: " + isRegistered);
		if (isRegistered)
		{
			...
			{
				LogHelper.v(TAG, "flag expired on: " + new Timestamp(expirationTime));
				return false;
			}
		}
		return isRegistered;
	...
	{
		LogHelper.d(TAG, "resetting backoff for " + context.getPackageName());
